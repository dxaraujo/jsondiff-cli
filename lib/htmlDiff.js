...
function mountHtmlDiff(fileTree) {
	...
	for (let nb in fileTree) {
		...
		for (let type in fileTree[nb]) {
			...
			const contentNode = createContentNode(dom, `/${nb}/${type}`, fileTree[nb][type])
...
function createContentNode(dom, path, content) {
	const node = dom.window.document.createElement('div')
	node.setAttribute('data-path', path)
	node.setAttribute('style', 'display: none;')
	node.setAttribute('class', 'vtv__code')
	node.setAttribute('tabindex', -1)
	node.appendChild(htmlToNode(dom, content))
	return node
}
...
const htmlDiff = (args) => {
	...
	const diffFileName = path.join(args.diffDir, 'diff.html')
